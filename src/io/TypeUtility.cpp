...
static const char* TAG = "TypeUtility";

int32_t TypeUtility::parseInt(const char* &token)
{
	token += std::strspn(token, " \t");
	int i = std::atoi(token);
	token += std::strcspn(token, " \t\r");
	return i;
}

uint32_t TypeUtility::parseUint(const char* &token)
{
	char* end;
...
float TypeUtility::parseFloat(const char* &token)
{
	token += std::strspn(token, " \t");
	float f = static_cast<float>(std::atof(token));
	token += std::strcspn(token, " \t\r");
	return f;
}

vec3i TypeUtility::parseInt3(const char* &token)
{
	vec3i index(0);
...
vec3u TypeUtility::parseUint3(const char* &token)
{
...
vec2f TypeUtility::parseFloat2(const char* &token)
{
...
vec3f TypeUtility::parseFloat3(const char* &token)
{
	float x = parseFloat(token);
	float y = parseFloat(token);
...
vec4f TypeUtility::parseFloat4(const char* &token)
{
	float x = parseFloat(token);
	float y = parseFloat(token);
	float z = parseFloat(token);
...
std::string TypeUtility::parseString(const char* &token)
{
	size_t length = std::strcspn(token, " \t");
#ifndef NDEBUG
...
std::vector<std::string> TypeUtility::split(const char* str, const char* delim)
{
	...
	while(*start)
	{
		size_t length = std::strcspn(start, delim);
...
std::vector<std::string> TypeUtility::split(const char* str, char delim)
{
	assert(str != nullptr);
...
std::vector<std::string> TypeUtility::split(const char* str)
{
