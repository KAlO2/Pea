...
//layout(location = 20) uniform vec3 lightColor;
...
void main()
{
	...
	float material_shininess = material.specular_shininess.w;
	
	vec3 viewDirection = normalize(viewPosition - _position);
	vec3 reflection = reflect(incident, normal);
	float Ls = pow(max(dot(viewDirection, reflection), 0.0), material_shininess);
	
	vec3 ambient = material.ambient * light0.ambient;
	vec3 diffuse = Ld * material.diffuse * light0.diffuse;
	vec3 specular= Ls * material_specular * light0.specular;
